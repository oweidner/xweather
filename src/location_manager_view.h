 ...
 * pops the window up. The search box, Add button, each row's checkbox, and
 * the Remove button only edit this in-window copy -- nothing outside the
 * window is touched until Apply. Apply invokes the registered callback
 * (unless the list is empty) and closes the window, same as Cancel; Cancel
 * (or the window's own close button) just closes it, discarding any edits -- the
